...
- Auth: static `COMPILER_API_TOKEN` bearer shared between Vercel edge and Fly (shipped simplification of the originally-planned Supabase JWT pass-through)
...
services/compiler-api/auth.py              # NEW (static bearer auth)
...
- `COMPILER_API_TOKEN` (static bearer for inter-service calls)
- `STRIPE_SECRET_KEY`
...
- `COMPILER_API_TOKEN` (shared bearer; must match the value Vercel uses)
- `LIBRARY_DB_PATH` (default `/data/library.db`; seeded from `/data-seed/library.db` on container start)

Note: this plan's earlier draft assumed Supabase JWT verification in `auth.py` (with `SUPABASE_URL` / `SUPABASE_SERVICE_ROLE_KEY` / `SUPABASE_JWT_SECRET` on Fly). The shipped implementation simplified to static bearer auth — those Supabase secrets are not required on the Fly side.
...
fly secrets set --app hcc-compiler-api COMPILER_API_TOKEN="$(zsvault get compiler-api-token)"
